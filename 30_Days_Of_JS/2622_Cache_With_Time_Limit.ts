...
class TimeLimitedCache {
  ...
  set(key: number, value: number, duration: number): boolean {
    const now = Date.now();
    const existsIndex = this.objects.findIndex(
      (obj) => obj.key === key && obj.expires > now
    );

    if (existsIndex !== -1) {
      this.objects[existsIndex].value = value;
      this.objects[existsIndex].expires = now + duration;
      return true;
    }
  ...
  get(key: number): number {
    const now = Date.now();
    return (
      this.objects.find((obj) => obj.key === key && obj.expires > now)?.value ||
      -1
    );
  }
  ...
  count(): number {
    const now = Date.now();
    return this.objects.filter((obj) => obj.expires > now).length;
  }
}
